# Graph Report - re-puestos  (2026-06-30)

## Corpus Check
- 15 files · ~40,574 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 315 nodes · 542 edges · 22 communities (19 shown, 3 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 48 edges (avg confidence: 0.81)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `0e651985`
...
1. `get_connection()` - 42 edges
2. `BaseHandler` - 41 edges
...
- `get_brand_models()` --calls--> `get_connection()`  [INFERRED]
  app.py → db.py
- `get_compat_years()` --calls--> `get_connection()`  [INFERRED]
  app.py → db.py
...
Cohesion: 0.07
Nodes (34): AvisosHandler, BaseHandler, BusquedaHandler, CatalogoHandler, DashboardHandler, EditarProductoHandler, EstadoPublicacionHandler, FavoritosHandler (+26 more)
...
Cohesion: 0.12
Nodes (11): get_market_prices(), get_nav_categories(), haversine_km(), NuevoProductoHandler, OnboardingHandler, PerfilTiendaHandler, PublicarAvisoHandler, PublicarMotoHandler (+3 more)

### Community 5 - "Auth UI y Layout Base"
Cohesion: 0.67
Nodes (3): Sellers/Tiendas data (company_name/whatsapp/rating/lat-lng), Leaflet OpenStreetMap store locator, tiendas.html (stores map)
...
Cohesion: 0.40
Nodes (4): get_brand_models(), get_compat_years(), Mapa marca -> modelo -> [anio_min, anio_max] declarado por los vendedores     (, Mapa marca -> modelos, para el selector de compatibilidad del home.
...
## Knowledge Gaps
- **114 isolated node(s):** `Added`, `Changed`, `Added`, `Changed`, `Added` (+109 more)
...
- **Why does `get_connection()` connect `Modelo de Datos SQLite` to `App Tornado y Rutas`, `DB, Seeding y Arranque`, `Community 20`, `Sistema de Iconos y Ticker`?**
...
- **Are the 37 inferred relationships involving `get_connection()` (e.g. with `.get_current_user()` and `.get()`) actually correct?**
  _`get_connection()` has 37 INFERRED edges - model-reasoned connections that need verification._
- **What connects `Mapa marca -> modelos, para el selector de compatibilidad del home.`, `Mapa marca -> modelo -> [anio_min, anio_max] declarado por los vendedores     (`, `Particulares/avisos: punto aproximado (~circulo de privacidad), no la direccion` to the rest of the system?**
